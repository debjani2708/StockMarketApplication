...
public interface StockPriceRepository extends JpaRepository<StockPriceDetails, Long> {
	...
	@Query("select cd FROM StockPriceDetails cd WHERE cd.companyCode=?1 AND cd.stockPriceDate BETWEEN ?2 AND ?3")
	public List<StockPriceDetails> findStockByCompanyCodeInDateRange(Long companyCode,LocalDate from, LocalDate to);
}